...
class FeatureDropoutFunction(torch.autograd.function.InplaceFunction):
    ...
    def forward(cls, ctx, input, p=0.5, train=False, inplace=False):
        ...
        if ctx.p > 0 and ctx.train:
            ctx.noise = input.new().resize_(input.size(0), input.size(2))
            if ctx.p == 1:
                ctx.noise.fill_(0)
            else:
                ctx.noise.bernoulli_(1 - ctx.p).div_(1 - ctx.p)
            # ctx.noise = ctx.noise[batch_idxs.batch_idxs_torch, :]
            ctx.noise = torch.stack([ctx.noise] * input.size(1), dim=1)
...
class MultiLevelEmbedding(nn.Module):
    ...
    def forward(self, xs, extra_content_annotations=None):
        content_annotations = [
            emb_dropout(emb(x))
        ...
        batch_size = content_annotations.shape[0]
        seq_len = content_annotations.shape[1]
        timing_signal = torch.stack([self.position_table[:seq_len, :]] * batch_size, dim=0)
        timing_signal = self.timing_dropout(timing_signal)

        # Combine the content and timing signals
        if self.partitioned:
            annotations = torch.cat([content_annotations, timing_signal], 2)
        else:
...
class Encoder(nn.Module):
    ...
    def forward(self, xs, batch_idxs, extra_content_annotations=None):
        ...
        res, timing_signal = emb(xs, extra_content_annotations=extra_content_annotations)
